...
mongoConfig
  ...
  .then(() => {
    ...
    app.listen(port, () => {
      console.log(
        `Server listening on port ${port} for requests from http://localhost:3000, and connected to database cluster`
      );
    });
  })
  .catch((error) => {
    //Handle any error that occurs while attempting to start the server
    console.error(
      "An error occured while attempting to start the server: " + error
    );
  });
